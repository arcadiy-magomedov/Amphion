...
/// Returns the exact non-negative rational square root when one exists.
#[must_use]
pub(super) fn exact_rational_sqrt(value: &BigRational) -> Option<BigRational> {
    if value.is_negative() {
        return None;
    }
    let numerator_root = bigint_isqrt(value.numer());
    let denominator_root = bigint_isqrt(value.denom());
    if &numerator_root * &numerator_root != *value.numer()
        || &denominator_root * &denominator_root != *value.denom()
    {
        return None;
    }
    Some(BigRational::new_raw(numerator_root, denominator_root))
}
