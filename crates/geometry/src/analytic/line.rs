...
//! `p(t) = origin + t · direction`, where `direction` is the exact finite
//! non-zero vector supplied by the caller. Its binary components and magnitude
//! are preserved: the magnitude carries the affine parameter scale needed to
//! synchronize a 3-D edge curve with its 2-D p-curve. The domain is the full
//! real line (no bounds), so only finiteness of `t` is checked.
//!
//! Derivatives:
//! - `p′(t) = direction`
//! - `p″(t) = 0`
...
use amphion_foundation::{Point2, Point3, SchemaVersion, Transform3, Vector2, Vector3};
...
use super::{
    ...
    helpers::{
        ...
        exact_affine_eval3, exact_line_project2, exact_line_project3, mag3, scale3,
    },
    transform::{exact_transform_point, exact_transform_vector, is_identity_transform},
};
...
/// `direction` is the stored finite non-zero affine coefficient. Its two
/// components may carry different parameter-space dimensions. The domain is
/// `(−∞, +∞)`.
...
impl Line2 {
    /// Constructs a line while preserving the exact finite direction bits and
    /// their affine parameter scale.
    ///
    /// # Errors
    ///
    ...
    /// Returns the stored finite direction vector used by the parameterization.
    #[must_use]
...
///
/// The line is parameterized as `p(t) = origin + t · direction`, where
/// `direction` is the stored finite non-zero affine coefficient. The domain
/// is `(−∞, +∞)`.
#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
...
impl Line3 {
    ...
    ///   origin or direction component is not representable as `f64`
    ...
    pub fn try_transform(&self, transform: &Transform3) -> Result<Self, TransformError> {
        ...
        let o = exact_transform_point(transform, self.origin)?;
        let d_vec = exact_transform_vector(transform, self.direction)?;
...
mod tests {
    ...
    use crate::analytic::{Cone, Plane};
    use crate::traits::{Curve2Evaluator, Curve3Evaluator, SurfaceEvaluator};
    ...
    #[test]
    fn line2_construction_preserves_direction_scale() {
        let line = Line2::try_new(
            Point2::try_new(0.0, 0.0).unwrap(),
            Vector2::try_new(3.0, 4.0).unwrap(),
        )
        .unwrap();
        assert_eq!(line.direction().into_array(), [3.0, 4.0]);
    }
    ...
    fn line2_exact_distance_cancellation_regression() {
        ...
        // helper, using the same stored direction.
    ...
    #[test]
    fn line2_serde_preserves_non_unit_direction() {
        let line: Line2 = serde_json::from_str(
            r#"{"version":{"major":1,"minor":0},"origin":{"x":1.0,"y":2.0},"direction":{"x":2.0,"y":0.0}}"#,
        )
        .unwrap();
        assert_eq!(line.direction().into_array(), [2.0, 0.0]);
    }

    #[test]
    fn line2_serde_rejects_zero_direction() {
        assert!(
            serde_json::from_str::<Line2>(
                r#"{"version":{"major":1,"minor":0},"origin":{"x":1.0,"y":2.0},"direction":{"x":0.0,"y":0.0}}"#
            )
            .is_err()
        );
    }
    ...
    #[test]
    fn line3_construction_preserves_direction_scale() {
        let line = Line3::try_new(
            Point3::try_new(0.0, 0.0, 0.0).unwrap(),
            Vector3::try_new(2.0, 3.0, 4.0).unwrap(),
        )
        .unwrap();
        assert_eq!(line.direction().into_array(), [2.0, 3.0, 4.0]);
    }
    ...
    #[test]
    fn line3_serde_preserves_non_unit_direction() {
        let line: Line3 = serde_json::from_str(
            r#"{"version":{"major":1,"minor":0},"origin":{"x":1.0,"y":2.0,"z":3.0},"direction":{"x":2.0,"y":0.0,"z":0.0}}"#,
        )
        .unwrap();
        assert_eq!(line.direction().into_array(), [2.0, 0.0, 0.0]);
    }

    #[test]
    fn line3_serde_rejects_zero_direction() {
        assert!(
            serde_json::from_str::<Line3>(
                r#"{"version":{"major":1,"minor":0},"origin":{"x":1.0,"y":2.0,"z":3.0},"direction":{"x":0.0,"y":0.0,"z":0.0}}"#
            )
                .is_err()
        );
    }
    ...
    fn line3_try_transform_identity_is_noop() {
        let l = Line3::try_new(
            Point3::try_new(-0.0, 2.0, 3.0).unwrap(),
        ...
        let out = l
            ...
            .unwrap();
        assert_eq!(out, l);
        assert_eq!(out.origin().x().to_bits(), (-0.0_f64).to_bits());
        assert_eq!(out.direction().y().to_bits(), (-0.0_f64).to_bits());
    }

    #[test]
    fn exact_transform_keeps_plane_pcurve_synchronized() {
        let a = 1.649_161_156_696_929;
        let b = 1.236_870_867_522_697;
        let pcurve = Line2::try_new(
            Point2::try_new(0.0, 0.0).unwrap(),
            Vector2::try_new(a, b).unwrap(),
        )
        .unwrap();
        let curve = Line3::try_new(
            Point3::try_new(0.0, 0.0, 0.0).unwrap(),
            Vector3::try_new(a, b, 0.0).unwrap(),
        )
        .unwrap();
        ...
        .unwrap();
        let transformed_curve = curve.try_transform(&transform).unwrap();
        let transformed_plane = plane.try_transform(&transform).unwrap();

        let parameter = 2.0_f64.powi(40);
        let uv = pcurve
            .evaluate(parameter, DerivativeOrder::Position, &ctx())
            .unwrap()
            .position
            .into_array();
        let curve_point = transformed_curve
            .evaluate(parameter, DerivativeOrder::Position, &ctx())
            .unwrap()
            .position
            .into_array();
        let surface_point = transformed_plane
            .evaluate(uv[0], uv[1], DerivativeOrder::Position, &ctx())
            .unwrap()
            .position
            .into_array();
        assert_eq!(curve_point, surface_point);
        assert_eq!(curve_point[0].to_bits(), (-2.0_f64.powi(-11)).to_bits());
    }
    ...
    fn line3_try_transform_scale_rotation_translation() {
        ...
        // direction (1,0,0) -> (0,2,0), preserving affine parameter scale.
    ...
    #[test]
    fn line3_try_transform_rejects_unrepresentable_result() {
        // The exact transformed point lies beyond the finite f64 range even
        // though the transform itself contains only finite values.
        let huge = f64::MAX;
        let m = [
        ...
        assert_eq!(
            l.try_transform(&t),
            Err(super::TransformError::UnrepresentableResult)
        );
    }

    // ─── Typed parameter tolerance regression tests ──────────────────────────

    /// A cone generator and its p-curve must identify the same model-space
    /// point at the same parameter; independently normalizing either direction
    /// destroys that synchronization.
    #[test]
    fn line_parameter_scale_keeps_cone_pcurve_synchronized() {
        let pcurve = Line2::try_new(
            Point2::try_new(0.0, 0.0).unwrap(),
            Vector2::try_new(0.0, 2.0).unwrap(),
        )
        .unwrap();
        let curve = Line3::try_new(
            Point3::try_new(0.0, 0.0, 0.0).unwrap(),
            Vector3::try_new(2.0, 0.0, 2.0).unwrap(),
        )
        .unwrap();
        let cone = Cone::try_new(
            Point3::try_new(0.0, 0.0, 0.0).unwrap(),
            Vector3::try_new(0.0, 0.0, 1.0).unwrap(),
            FRAC_PI_4,
            Vector3::try_new(1.0, 0.0, 0.0).unwrap(),
        ...
        let uv = pcurve
            ...
            .position
            .into_array();
        let curve_point = curve
            .evaluate(parameter, DerivativeOrder::Position, &ctx())
            .unwrap()
            .position;
        let surface_point = cone
            .evaluate(uv[0], uv[1], DerivativeOrder::Position, &ctx())
            .unwrap()
            .position;
        assert!(
            dist3(curve_point, surface_point) < 1e-12,
            "3-D curve and p-curve disagree at the same parameter"
        );
    }

    /// A Curve2 evaluation is certified in parameter-space coordinates, not
    /// against the unrelated model-space length tolerance.
    #[test]
    fn line2_evaluation_uses_parametric_tolerance() {
        let line = Line2::try_new(
            Point2::try_new(0.0, 0.0).unwrap(),
            Vector2::try_new(0.1, 0.1).unwrap(),
        )
        .unwrap();
        let tight_context = EvaluationContext::unlimited(
            ToleranceContext::try_new(1.0, 0.0, 1e-10, 1e-300).unwrap(),
        );
        let result = line.evaluate(0.1, DerivativeOrder::Position, &tight_context);
        assert!(
            matches!(result, Err(GeometryError::Uncertified { .. })),
            "expected the UV error bound to exceed parametric tolerance: {result:?}"
        );
    }

    /// Blocker 1 regression: Line2 origin=(0,0), dir=(2,3), q=(1,0),
